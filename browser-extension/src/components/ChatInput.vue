...
const currentModel = computed(() => {
  return thinkingModeEnabled.value ? 'Gemini 2.5 Pro' : 'Gemini 2.5 Flash Lite'
})
        ...
        <div class="flex items-center space-x-2">
          ...
          >
            ...
            :class="`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 ${
                thinkingModeEnabled ? 'translate-x-[16px]' : 'translate-x-[0px]'
                }`"
            />
          </button>
          <span class="text-xs text-gray-700 ml-1">{{ t('thinkingMode') }}</span>
        </div>
